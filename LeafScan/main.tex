...
\Author{Rasyad Bimasatya$^{1}$, Mario Valerian Rante Ta’dung$^{2}$, Dewa Ayu Eka Natalia Pratiwi$^{3}$, Rahmatia$^{4}$, Evan Pandu Nata$^{5}$, A M Fauzan Baihaqi T$^{6}$, and A. Afif Alhaq$^{7}$}
...
\address{%
$^{*}$ \quad Information System, Department of Mathematics, Faculty of Mathematics and Natural Science, Hasanuddin University \\}
...
\begin{enumerate}[label=\alph*)]
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{Images/activity_recomendation.png}
...
\begin{itemize}
    ...
    Pengembangan \textit{frontend} aplikasi \textit{LeafScan} menggunakan \textit{framework Flutter} untuk menciptakan antarmuka yang intuitif dan responsif. Beberapa fitur \textit{frontend} yang sudah dikerjakan antara lain:
     ...
     \begin{itemize}
            ...
            \end{minipage}
        \end{figure}
        \item Tampilan Deteksi
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.3\textwidth]{Images/detection_view.png}
